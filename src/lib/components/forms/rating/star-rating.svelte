<script lang="ts">
	...
	onMount(() => {
		...
		const myRater = rater({
			...
			rateCallback: function rateCallback(rating: any, done: any) {
			...
			rating: currentRating // Valor inicial
		});
	});
</script>
...
<div class="form-item">
	<label class="label form-label {$$restProps.required ? 'required-label' : ''}">
		...
		<div class="flex start acenter gap-smaller">
			...
			<p class="rating-selected">{currentRating}</p>
		</div>

		<input type="hidden" name="{name}-rating" value={currentRating} />
	</label>
</div>
