...
export const Helpers = {
    ...
    getSelectorItem(data: FormData, field?: string) {
        const number = field ? data.get(`selected-${field}-resource-id`) : data.get('selected-resource-id');
        return {
            id: Number(number),
            name: (field ? data.get(`selected-${name}-resource-name`) : data.get('selected-resource-name')) as string,
        };
    },
