...
export const load = async ({ url, locals }) => {
	...
	post.visible = Boolean(isUpdate ? post.visible : true);
	const categories = await repository.get({
		distinct: ['category'],
		select: {
			category: true
		}
	});
	const everyCategory = categories.flatMap((c) => c.category?.split(',') || []).map(c => c.trim());
	const uniqueCategories = [...new Set(everyCategory)];
...
export const actions = {
	default: async ({ request, url }) => {
		...
		const formAction = datas.get('_action');
		...
		const categories = datas.getAll('categories[]').map((c) => c.toString().trim());
