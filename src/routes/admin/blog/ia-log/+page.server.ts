...
export const load = async ({ url }) => {
    ...
    const yearUsage: any = await connection.$queryRaw`SELECT model, YEAR(created_at) AS created, SUM(total_tokens) AS tokens FROM ia_log GROUP BY model, YEAR(created_at) ORDER BY created DESC, model ASC`;
    const yearlyUsage = yearUsage.map((entry: any) => ({
        model: entry.model,
        year: entry.created,
        tokens: Number(entry.tokens)
    }));
