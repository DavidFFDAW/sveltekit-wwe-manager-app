...
export const actions = {
    upsert: async ({ request, locals }) => {
        ...
        if (!Helpers.hasPermission(locals)) return Helpers.error('No tienes permisos para realizar esta acción', 403);
        ...
        try {
            ...
            const datas = {
                ...
                won_date: formData.get('start_date') ? new Date(formData.get('start_date') as string) : new Date(),
            ...
            if (isTagTeam) {
                ...
                upsertObject = {
                    ...
                    Team: teamId ? { connect: { id: teamId } } : action === 'update' ? { disconnect: true } : undefined,
                };
            }
            ...
            if (datas.is_active) {
                ...
                if (activeReign) {
                    const updatedOldReignDays = ReignUtils.getDaysBetweenDates(activeReign?.won_date, datas.won_date);
                    console.log({
                        activeReign,
            ...
            const upsertAction =
                action === 'create'
                    ...
                    : () =>
                          reignsRepo.updateById(Number(updateId), upsertObject as Prisma.ChampionshipReignUpdateInput);
