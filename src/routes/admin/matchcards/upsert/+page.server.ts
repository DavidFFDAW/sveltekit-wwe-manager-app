...
export const actions = {
	default: async ({ request }) => {
		...
		try {
			...
			const slugId = Number(formData.get('slug') as string);
			...
			const ppvCard = isUpdate
				? await matchCardRepository.update({ id: slugId }, ppvDatas)
				: await matchCardRepository.create(ppvDatas);
